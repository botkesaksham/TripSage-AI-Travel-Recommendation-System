def FINAL(Type,Duration,Budget,TYPE,Ques):
  ...
  J_priority_df = pd.read_csv('data/Chennai_priority_mapping.csv')
  ...
  lat_lng_df.set_index(chennai_poi_df.index,inplace=True)
  ...
  #chennai_Hotels_df.set_index('HID',inplace=True)
  ...
  def threshold_dis(n,lis_of_distances): # n - threshold in km
    ...
    for i in range(0,len(lis_of_distances)):
      if (lis_of_distances[i]<=float(n)):
        c=c+1
        #print(i,': ',lis_of_distances[i],'\t',chennai_Hotels_df.iloc[i,0])
  ...
  sorted_by_des = chennai_Hotels_df.sort_values(by=['Desirable','Price'],ascending=False,inplace=False)
  ...
  def hid_suitable(hid):
    IND = id_to_ind(hid)
    PRICE = chennai_Hotels_df.loc[IND,'Price']
  ...
  # got one_row with some distances 
    ...
    for j in dist_pri: #[0.5,0.75,1.2,3.5,6.0,10.0,20.0,35.0,50.0]
      ...
      for k in dist_selected:   ## TRY EMPTY CHANGE
        price_dict[k]=chennai_Hotels_df.loc[id_to_ind(k),'Price']
  ...
  ## Map
  #nearest coord, selected coord list, chennai poi df, lat_lng_ddf, all_routes

# Define the necessary functions and variables here, assuming they are already provided elsewhere in your script

  def get_pid(place):
      return extract(np.array_str(chennai_poi_df[chennai_poi_df['POIs'] == place].index.values))
...
# Initialize an empty list to store the itinerary details
  big_la = []
...
# User information
  user_info = user_info()  # Assuming `user_info` is a function that provides user-related information
